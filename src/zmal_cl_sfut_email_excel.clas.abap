...
* Needed to send emails
    DATA: bcs_exception        TYPE REF TO cx_bcs,
          ...
          wa_attachment_header TYPE LINE OF soli_tab,
          attachment_subject   TYPE sood-objdes,

          mail_title           TYPE so_obj_des,
          t_mailtext           TYPE soli_tab,
          wa_mailtext          TYPE LINE OF soli_tab,
    ...
    create_attachment(
      IMPORTING
        ex_rawdata   = t_rawdata
        ex_bytecount = sood_bytecount
    ).
...
* set sender in case if no own email is availabe
        cl_sender  = cl_cam_address_bcs=>create_internet_address( 'webereduardo@johndeere.com' ).
        cl_send_request->set_sender( cl_sender ).

* add recipient(s) - here only 1 will be needed
*        send_to = p_email.
        IF send_to IS INITIAL.
          send_to = 'webereduardo@johndeere.com'.  " Place into SOST in any case for demonstration purposes
        ENDIF.
