CLASS ZMA0_CL_SFUT_EMAIL_HELPER DEFINITION PUBLIC FINAL CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING VALUE(re_helper)
                  TYPE REF TO ZMA0_CL_SFUT_EMAIL_HELPER.
  ...
  PRIVATE SECTION.
    ...
    DATA purch_org TYPE ekko-ekorg.
...
CLASS ZMA0_CL_SFUT_EMAIL_HELPER IMPLEMENTATION.

  METHOD constructor.

* The subject and body of the e-mail will be defined for each specific
* purchasing group
    me->purch_org = im_purch_group.
    me->langu = im_langu.
...
* for that specific purchase group
    CALL FUNCTION 'READ_TEXT'
      EXPORTING
        id                      = ZMA0_CL_SFUT_EMAIL_HELPER=>text_id
        language                = me->langu
        name                    = me->get_object_name( )
        object                  = ZMA0_CL_SFUT_EMAIL_HELPER=>object_type
      TABLES
        lines                   = re_lines
      EXCEPTIONS
        id                      = 1
...
* Delete all text components related to that purchasing group e-mail
  ...
  METHOD open_text_editor.
    ...
    lw_header-tdobject   = ZMA0_CL_SFUT_EMAIL_HELPER=>object_type.
    lw_header-tdname     = me->get_object_name( ).
    lw_header-tdid       = ZMA0_CL_SFUT_EMAIL_HELPER=>text_id.
    lw_header-tdspras    = me->langu.
    lw_header-tdlinesize = ZMA0_CL_SFUT_EMAIL_HELPER=>max_line_size.
...
* lose track of the already created objects on the application server
    re_object_name = |ZMA0_SFUT_EMAIL_{ me->purch_org }|.
